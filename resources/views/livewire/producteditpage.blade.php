                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="row">
                                        ...
                                        <select class="form-control" id="category" name="category">
                                            ...
                                            @foreach ($productcate as $item)
                                                @if($item->type === 'Product')
                                                    <option value="{{ $item->value }}" {{ $productedit->category == $item->value ? 'selected' : '' }}>
                                                        {{ $item->value }}
                                                    </option>
                                                @endif
                                            @endforeach
                                    ...
                                    <div class="mb-2 col-md-4">
                                        <label for="vehicle" class="mb-2">Vehicle Category</label>
                                        <span style="color:red;">*</span>
...
@php
    $selectedArray = isset($selectedVehicles)
        ? (is_array($selectedVehicles) ? $selectedVehicles : explode(',', $selectedVehicles))
        : [];
@endphp

<select class="form-control" id="vehicle" name="vehicle[]" multiple>
    <option class="text-center" disabled>---Select Vehicle Category----</option>
    @foreach ($category as $item)
        @if($item->type === 'vehicle')
            <option value="{{ $item->value }}" 
                {{ in_array($item->value, $selectedArray) ? 'selected' : '' }}>
                {{ $item->value }}
            </option>
        @endif
    @endforeach
                                    ...
                                    <div class="mb-2 col-md-4">
                                        <label for="quantity" class="mb-2">Per Box Pcs Quantity</label>
                                        <input class="form-control" type="text" id="quantity" name="quantity" value="{{$productedit->quantity}}">
                                    </div>

                                    <div class="mb-2 col-md-4">
                                        <label for="measurement" class="mb-2">Measurement</label>
                                        <select class="form-control" id="measurement" name="measurement">
                                            <option class="text-center" disabled>---Select Measurement Category----</option>
                                            <option value="Ltr" {{ $productedit->measurement == 'Ltr' ? 'selected' : '' }}>Ltr</option>
                                            <option value="PCS" {{ $productedit->measurement == 'PCS' ? 'selected' : '' }}>PCS</option>
                                            <option value="BKT" {{ $productedit->measurement == 'BKT' ? 'selected' : '' }}>BKT</option>
                                        </select>
                                    </div>

                                    <div class="mb-2 col-md-4">
                                        <label for="box" class="mb-2">Total Box/Bkit Qty</label>
                                        <input class="form-control" type="text" id="box" name="weightnum" value="{{$productedit->boxquantity}}">
                                    </div>

                                    <div class="mb-2 col-md-4">
                                        <label for="weightnum" class="mb-2">Weight (GMS) Of Per Box</label>
                                        <input class="form-control" type="text" id="weightnum" name="weightnum" value="{{$productedit->weightnum}}" placeholder="eg.300" oninput="this.value = this.value.replace(/[^0-9]/g,'')">
                                    </div>

                                    <div class="mb-2 col-md-4">
                                        <label for="weightclass" class="mb-2">Weight Of Per Box Class</label>
                                            <select class="form-control" id="weightclass" name="weightclass">
                                                <option class="text-center" value="">---Select Class Category----</option>
                                                @foreach ($productcate as $item)
                                                    @if ($item->type === 'measurement')
                                                        <option value="{{ $item->value }}" 
                                                        {{ $productedit->weightclass == $item->value ? 'selected' : '' }}>
                                                            {{ $item->value }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                    ...
                                    <div class="mb-2 col-md-4">
                                        <label for="hsncode" class="mb-2">Product HSN Code</label>
                                    ...
                                    <div class="mb-2 col-md-12" style="display: none;">
                                        <label for="metatag" class="mb-2">Meta Tag Title</label>
                                    ...
                                    <div class="mb-2 col-md-12" style="display: none;">
                                        <label for="metadescription" class="mb-2">Meta Tag Description</label>
                                        <textarea class="form-control" id="metadescription" name="metadescription"></textarea>
                                    </div>

                                    <div class="mb-2 col-md-12" style="display: none;">
                                        <label for="metakeyword" class="mb-2">Meta Tag Keywords</label>
                                    ...
                                    <div class="row" data-index="0">
                                        ...
                                        @foreach ($userrole as $index => $roledata)
                                            @php
                                                $matchedPrice = $productprice->firstWhere('role', $roledata->id);
                                            @endphp
                                            <div class="row mb-2">
                                                <div class="col-md-3">
                                                    <span style="color:red;">*</span>
                                                ...
                                                <div class="col-md-4">
                                                    <input class="form-control productprice" type="text"
                                                           id="productprice_{{ $index }}"
                                                           name="price[]"
                                                           value="{{ $matchedPrice->price ?? '' }}"
                                                           placeholder="e.g. 200">
                                                </div>
                                                <div class="mb-2 col-md-4">
                                                    <input class="form-control totalbox" type="text"
                                                           id="totalbox_{{ $index }}"
                                                           name="totalprice[]"
                                                           value="{{ $matchedPrice->totalprice ?? '' }}"
                                                           placeholder="00.0"
                                                           readonly>
                                                </div>
                                            </div>
